...
> 注: 比べる相手を間違えかけました。前作には 949万文字の走行が2本あり、
> **文字単位が step 1,750、SentencePiece 8k が step 1,000** です。
> 今回は 8k サブワードなので相手は 1,000 のほうで、今回の 900 とほぼ同じでした。
> ただしコーパスの中身は違う（対話 → ウェブ+青空文庫）ので、
> ここで見ているのは絶対値ではなく**動く向き**です。
...
![再開の検査](docs/images/3lm-killtest.png)

**壊した対照だけが飛び抜けている**ことが大事です。
対照を置かないと、この検査に感度があるのかどうかが分かりません。
...
![口調を乗せる代償](docs/images/3lm-gal-cost.png)
